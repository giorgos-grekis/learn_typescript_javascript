...
type OK = IsMiddleAged<42>; // true
type KO = IsMiddleAged<40>; // false

// We get back never
type hmm = IsMiddleAged<never>;
...
type IsUser2<T> = T extends { name: string; team: { memberCount: number } } ? true : false;
...
type FirstAndLast<Tuple> = Tuple extends [infer Frist, ...any[], infer Last] ? [Frist, Last] : [];
...
/*** Type Assignments ***/
type AVeryHeavyTask<T> = unknown;
type Fn3<T> = AVeryHeavyTask<T> extends infer Result ? [Result, Result] : never;
