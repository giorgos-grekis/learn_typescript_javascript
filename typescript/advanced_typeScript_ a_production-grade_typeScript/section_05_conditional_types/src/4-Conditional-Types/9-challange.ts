import { Conditional } from "./2-challange";
import { Equal, Expect } from "..";

type UnwrapNestedArrays<ArrayType extends any[]> = ArrayType extends (infer ElementType)[][] ? ElementType[] : ArrayType;

function mergeNestedArrays<ArrayType extends any[]>(nestedArray: ArrayType): UnwrapNestedArrays<ArrayType> {
  return nestedArray.reduce((accumulator, item) => [...accumulator, ...(Array.isArray(item) ? item : [item])], []);
}
