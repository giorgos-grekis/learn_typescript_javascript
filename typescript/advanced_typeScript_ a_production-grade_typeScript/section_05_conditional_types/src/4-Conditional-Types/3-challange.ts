 ...
 */
export type FetchOrDefault<Key, Obj, Default> = Key extends keyof Obj ? Obj[Key] : Default;
 ...
 */
function fetchOrDefault<K extends string, O extends {}, D>(key: K, obj: O, defaultValue: D): FetchOrDefault<K, O, D> {
  return (obj as any)[key] ?? defaultValue;
}

// Example usage and tests

const result1 = fetchOrDefault("title", { title: "TypeScript Utility Types" }, "hello");
// Test case 1: Key exists in the object, should return the value of the key
type test1 = Expect<Equal<typeof result1, string>>;

const result2 = fetchOrDefault("oops", { title: "TypeScript Utility Types" }, undefined);
