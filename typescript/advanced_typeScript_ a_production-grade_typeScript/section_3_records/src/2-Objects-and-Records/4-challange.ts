 ...
 *                             👇                      */
export type OptionalizeId<T extends { id: unknown }> = Partial<Pick<T, "id">> & Omit<T, "id">;
...
type test1 = Expect<Equal<result1, { id?: number } & { name: string; age: unknown }>>;
...
type test2 = Expect<Equal<result2, { id?: string } & { title: string; content: string }>>;
