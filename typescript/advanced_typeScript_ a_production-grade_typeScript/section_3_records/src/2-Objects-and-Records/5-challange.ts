...
type result2 = MergeAndOverride<{ name: string; age: string; role: string }, { age: 42; role: "admin" }>;
// Expect result2 to be an object with properties: { name: string; age: 42; role: "admin" }
type test2 = Expect<Equal<result2, { name: string } & { age: 42; role: "admin" }>>;
...
type test3 = Expect<Equal<result3, { name: string; id: number } & { age: number }>>;
